...
$inputpath = ".\backup.csv" # 入力ファイルのパス

$inputurl="https://docs.google.com/spreadsheets/d/1sbaJpN3ljEqMtUl-m8pKqz7CQjfOE0kojmyluJ-HnIY/export?format=csv&gid=751114025" # 入力ファイルのURL

$textpath = ".\ILsgacha.txt" # 出力ファイルのパス
...
# $list = Get-Content -Path $inputpath -Encoding utf8NoBOM

# ファイルへのリンクから取得するなら↓をコメントアウト解除エクスプローラーををちょっと綺麗にしたい人用)
# Googleスプレッドシートからcsvをダウンロードするならhttps://docs.google.com/spreadsheets/d/<id>/export?format=csv&gid=<sheetid>
$list = Invoke-RestMethod -URI "$inputurl"

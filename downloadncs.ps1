...
(Get-Content -Path ".\invalids.txt") | ForEach-Object {
    ...
    do {
        ...
        $filename = Read-Host -Prompt "Enter filename(Full) or skip"
    } until ($filename -match "^[^ ].+\.mp3$|^skip$")
    if ($filename -notmatch "^skip$") {
        $filename = $filename.Substring(0, $filename.Length - 18)
        if ($_ -match "^i_[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$") {
            ffmpeg -hide_banner -loglevel -8 -vn -i ".\${_}.mp3" -map "0:0" -c copy -metadata title="${filename} (Instrumental)" ".\musics\${_}.mp3"
        } else {
            ffmpeg -hide_banner -loglevel -8 -vn -i ".\${_}.mp3" -map "0:0" -c copy -metadata title="${filename}" ".\musics\${_}.mp3"
        }
    }
}
Read-Host
